...
for task_id in list(problems)[40:]:  
    ...
    while True:
        ...
        prompt_feedback = code + '\n' + feedback + '\n' + 'Based on the test and feedback information code, only enter the modified code.'
...
write_jsonl("samples_codecot.jsonl", samples)
#######################################################
 
# first_task_id ="HumanEval/40" 

# prompt1 =  problems[first_task_id]["prompt"] 

# first_step = generate_first(prompt1)
...
#     feedback += "\nFailed tests:\n" + "\n".join(failed_tests)
#     # print(feedback)
#     # print('above is feedback----------------------------')

#     prompt_feedback=code+'\n'+feedback+'\n'+'Based on the test and feedback information code, only enter the modified code.'
#     # print(prompt_feedback)
#     # print('above is prompt_feedback----------------------------')
...
# samples.append({"task_id": first_task_id, "completion": code})

# completion = generate_one_competition(prompt1, first_step)    

# print(completion)

# samples.append({"task_id": first_task_id, "completion": completion})
...
#     first_step = generate_one_scot(problem_info["prompt"])
    

#     completion = generate_one_competition(problem_info["prompt"], first_step)

#     samples.append({"task_id": task_id, "prompt": problem_info["prompt"], "test": completion, "entry_point": "factorial"})
